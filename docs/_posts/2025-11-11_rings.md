...
We need to configure the interaction potential between particles and
...
These two quantities are used to create an object of type `RingsState`. We can do
...
also returns a geometry configuration, which we will talk about in the next section). Use example
```julia
...
p_chunks_cfg = ChunksCfg(num_x_chunks, num_y_chunks),
